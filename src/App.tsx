...
import{ReconciledPair,ReviewState,ReviewVerdict,applyVerdict,findNearbyPairs,loadReviewState,reconcileVerdicts,saveReviewState}from'./proximity';
import ProximityWorkspace from'./proximity.ui';
...
useEffect(()=>{const boot=loadUndo();if(boot==='invalid'||(boot&&!rows.some(r=>r.id===boot.id))){clearUndo();setUndoRaw(null);flash('err','最近编辑已无法撤销')}},
 ...
 []);const issues=useMemo(()=>issuesFor(rows),[rows]);
 // 近地点复核：候选对只依赖当前记录与半径；结论在渲染层按指纹核对，字段一改即回到待复核
 const nearPairs:ReconciledPair[]=useMemo(()=>reconcileVerdicts(findNearbyPairs(rows,review.radius),rows,review.verdicts),[rows,review.radius,review.verdicts]);
 // 返回是否写入成功：失败时只保留错误提示，调用方不得再提示成功，避免已确认的假象
 const setReview=(next:ReviewState):boolean=>{if(!saveReviewState(next)){flash('err','复核结论写入本地存储失败，界面未改变，请检查浏览器存储空间');return false}setReviewRaw(next);return true};
 const setRadius=(radius:number)=>setReview({...review,radius});
 const decidePair=(ids:[string,string],verdict:ReviewVerdict)=>{
   let verdicts:ReturnType<typeof applyVerdict>;
   try{verdicts=applyVerdict(rows,review.verdicts,ids,verdict)}catch{return flash('err','无法标记：其中一条记录已不存在')}
   if(!setReview({...review,verdicts}))return;
   flash('ok',verdict==='same'?'已标记为同点复采，结论绑定双方字段指纹并保存':'已标记为确为两份，结论绑定双方字段指纹并保存');
 };
const visible=rows.filter(r=>{const ri=issues.filter(x=>x.recordId===r.id);const status=ri.length?ri.some(x=>(r.resolutions[x.key]?.status||'pending')==='pending')?'pending':'resolved':'clean';return r.species.toLowerCase().includes(filters.species.toLowerCase())&&r.collector.toLowerCase().includes(filters.collector.toLowerCase())&&r.location.toLowerCase().includes(filters.location.toLowerCase())&&(filters.status==='all'||filters.status===status)});
 ...
 return <><header><div><span className="eyebrow">HERBARIUM DESK · 01</span><h1>植物标本标签<br/>预检台</h1><p>在打印之前，把每一份野外记录整理得准确、完整、可追溯。</p></div><button className="primary" onClick={()=>setEdit({...blank})}>＋ 新增标本</button></header><main>{msg&&<div role="alert" className={`toast ${msg.kind}`}>{msg.text}</div>}<section className="metrics"><article><span>馆藏记录</span><strong>{rows.length}</strong><small>当前工作集</small></article><article><span>待处理问题</span><strong>{issues.filter(i=>(rows.find(r=>r.id===i.recordId)?.resolutions[i.key]?.status||'pending')==='pending').length}</strong><small>需要人工复核</small></article><article><span>预检通过</span><strong>{rows.filter(r=>!issues.some(i=>i.recordId===r.id&&(r.resolutions[i.key]?.status||'pending')==='pending')).length}</strong><small>可进入打印</small></article><article><span>物种数</span><strong>{speciesCount(rows)}</strong><small>已识别名称</small></article></section>
 <nav className="worktabs"><button type="button" className={tab==='preflight'?'primary':''} aria-pressed={tab==='preflight'} onClick={()=>setTab('preflight')}>预检工作台</button><button type="button" className={tab==='proximity'?'primary':''} aria-pressed={tab==='proximity'} onClick={()=>setTab('proximity')}>近地点复核</button></nav>
 {tab==='proximity'?<ProximityWorkspace rows={rows} pairs={nearPairs} radius={review.radius} onRadius={setRadius} onDecide={decidePair}/>:<>
